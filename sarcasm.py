...
# ===================
train_df = pd.read_csv('/home/dse316/grp21/nlp/sarcasm _train .csv').dropna()
val_df = pd.read_csv('/home/dse316/grp21/nlp/sarcasm_val .csv').dropna()
...
BATCH_SIZE = 32
train_basic_ds = BasicTextDataset(train_texts, train_labels)
val_basic_ds = BasicTextDataset(val_texts, val_labels)
train_basic_dl = DataLoader(train_basic_ds, batch_size=BATCH_SIZE, shuffle=True)
val_basic_dl = DataLoader(val_basic_ds, batch_size=BATCH_SIZE)

train_bert_ds = BERTDataset(train_texts, train_labels)
val_bert_ds = BERTDataset(val_texts, val_labels)
train_bert_dl = DataLoader(train_bert_ds, batch_size=16, shuffle=True)
val_bert_dl = DataLoader(val_bert_ds, batch_size=16)
...
class RNNClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.fc(hidden[-1]), hidden[-1]  # output(logits), embedding
...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, x):
        emb = self.embedding(x).permute(1,0,2)  # (seq_len, batch, emb)
        out = self.transformer(emb)
        out = out.mean(dim=0)  # (batch, emb)
        return self.fc(out), out

# BERT for classification and embedding extraction
bert_model = BertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=2).to(device)
bert_encoder = BertModel.from_pretrained('bert-base-uncased').to(device)  # For embedding extraction
...
# =========================
# Validation: Predict, Get Embeddings, Compute F1
# =========================

def get_preds_embs_basic(model, val_dl):
    model.eval()
    ...
    return np.array(all_preds), np.array(all_labels), embs

def get_preds_embs_bert(model, encoder, val_dl):
...
# ==== Get preds, embeddings, F1 ====
rnn_preds, rnn_labels, rnn_embs = get_preds_embs_basic(rnn_model, val_basic_dl)
lstm_preds, lstm_labels, lstm_embs = get_preds_embs_basic(lstm_model, val_basic_dl)
trans_preds, trans_labels, trans_embs = get_preds_embs_basic(trans_model, val_basic_dl)
bert_preds, bert_labels, bert_embs = get_preds_embs_bert(bert_model, bert_encoder, val_bert_dl)
...
# Saving Embeddings
# =========================
torch.save({'embeddings': rnn_embs, 'labels': torch.tensor(rnn_labels)}, "rnn_val_embeddings.pt")
torch.save({'embeddings': lstm_embs, 'labels': torch.tensor(lstm_labels)}, "lstm_val_embeddings.pt")
torch.save({'embeddings': trans_embs, 'labels': torch.tensor(trans_labels)}, "trans_val_embeddings.pt")
torch.save({'embeddings': bert_embs, 'labels': torch.tensor(bert_labels)}, "bert_val_embeddings.pt")'''
...
# Load Data & Create Vocab
# ===================
train_df = pd.read_csv('/home/dse316/grp21/nlp/sarcasm_train.csv').dropna()
val_df = pd.read_csv('/home/dse316/grp21/nlp/sarcasm_val.csv').dropna()
train_texts = train_df['Sentence'].tolist()
train_labels = train_df['Tag'].astype(int).tolist()
val_texts = val_df['Sentence'].tolist()
